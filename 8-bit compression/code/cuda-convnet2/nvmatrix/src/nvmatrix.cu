...
void NVMatrix::addProduct(NVMatrix& a, NVMatrix &b, float scaleThis, float scaleAB, cudaStream_t stream) {
    ...
    */
    ...
    if(getNumCols() != 1000)
    {
    	...
    	compress8bit(data8bit, absMax, _buffer8bit);
    	decompress8bit(data8bit, absMax, _buffer8bit);

		cout << getNumRows() << "x" << getNumCols() << endl;
		cout << a.getNumRows() << "x" << b.getNumCols() << endl;
...
{
    assert(isContiguous());
    kTest<<<NUM_TILE_BLOCKS,NUM_TILE_THREADS_PER_BLOCK>>>(getDevData(), _numCols* _numRows);

    getLastCudaError("Kernel execution failed");
}

void NVMatrix::compress8bit(float *tbl, float absMax, unsigned char *out)
{
    assert(isContiguous());
	int blocks = (_numCols* _numRows/128) + 1;
    kCompression_8bit<<<blocks,128>>>(tbl, getDevData(), absMax, _numCols* _numRows, out);

    getLastCudaError("Kernel execution failed");
}

void NVMatrix::compress8bit_linear(float *tbl, float absMax, unsigned char *out)
{
    assert(isContiguous());
	int blocks = (_numCols* _numRows/128) + 1;
    kCompression_8bit_linear<<<blocks,128>>>(tbl, getDevData(), absMax, _numCols* _numRows, out);

    getLastCudaError("Kernel execution failed");
}

void NVMatrix::compress8bit_standard(float *tbl, unsigned char *out, float lower, float upper)
{
    assert(isContiguous());
	int blocks = (_numCols* _numRows/128) + 1;
    kCompression_8bit_standard<<<blocks,128>>>(tbl, getDevData(), _numCols* _numRows, out, lower, upper);

    getLastCudaError("Kernel execution failed");
}


void NVMatrix::decompress8bit(float *tbl, float absMax, unsigned char *in)
{
    assert(isContiguous());

	int blocks = (_numCols* _numRows/128) + 1;
    kDecompression_8bit<<<blocks,128>>>(tbl, in, absMax, _numCols* _numRows, getDevData());

    getLastCudaError("Kernel execution failed");
}

void NVMatrix::decompress8bit_standard(float *tbl, unsigned char *in)
{
    assert(isContiguous());

	int blocks = (_numCols* _numRows/128) + 1;
    kDecompression_8bit_standard<<<blocks,128>>>(tbl, in, _numCols* _numRows, getDevData());
...
void NVMatrix::abs(NVMatrix& target) {
    apply(NVMatrixOps::Abs(), target);
}
